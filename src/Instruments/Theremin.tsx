...
const sketch = (p: P5CanvasInstance<ThereminProps>) => {
  let isMousePressedAndOnCanvas = false;
  ...
  const canvasPressed = () => {
    ...
    playTheremin(sc_mouseX, sc_mouseY);
    isMousePressedAndOnCanvas = true;
  };

  const releaseNote = () => {
    isMousePressedAndOnCanvas = false;
  ...
  const playTheremin = (x: number, y: number) => {
    ...
    if (cellNum !== prevCell) {
      ...
      if (synth.envelope.sustain !== 0) synth.frequency.rampTo(pitch, 0.3);
        ...
        isMousePressedAndOnCanvas ||
        (thereminState === 'playback' && sequence.mouseOn[sequenceCounter])
      ) {
        synth.triggerAttackRelease(pitch, '8n', '+0.02');
      }
    }
    if (!prevCell) {
      pitch = notes[cellNum];
